...
export async function registerRoutes(
  ...
  app.post("/api/bookings", async (req, res) => {
    try {
      ...
      console.log(
        `[booking] Saved #${booking.id} for ${booking.customerEmail} ` +
        `(tour ${booking.tourId}, ${booking.travelDate})`,
      );
      ...
      // A booking that was not written must never look like one that was.
      console.error("[booking] FAILED TO SAVE — request body follows:", error);
      console.error("[booking] Unsaved enquiry:", JSON.stringify(req.body));
      return res.status(503).json({
        error:
          "We could not save your booking just now. Nothing has been reserved — " +
          "please call us on +255 123 456 789 or try again in a few minutes.",
      });
    }
  });
  ...
  app.post("/api/contact", async (req, res) => {
    try {
      ...
      // Same rule as bookings: never report success for an unsaved enquiry.
      console.error("[contact] FAILED TO SAVE:", error);
      console.error("[contact] Unsaved message:", JSON.stringify(req.body));
      return res.status(503).json({
        error:
          "We could not send your message just now. Please email " +
          "hello@japhazelsafaris.com or try again shortly.",
      });
    }
  });
