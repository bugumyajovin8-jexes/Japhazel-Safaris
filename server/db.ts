...
/**
 * Hosted Postgres (Neon, Supabase, Vercel) requires TLS, so production defaults
 * to it. An explicit `sslmode` in the connection string always wins, which is
 * what lets a self-hosted or local Postgres without TLS run the production
 * build: append `?sslmode=disable`.
 */
function resolveSsl(): false | { rejectUnauthorized: boolean } {
  const mode = connectionString?.match(/[?&]sslmode=([^&]+)/)?.[1];
  if (mode === "disable") return false;
  if (mode) return { rejectUnauthorized: mode === "verify-full" };
...
export const pool = new Pool({
  ...
  // Serverless invocations are short-lived and each gets its own pool, so a
  // large per-instance pool just burns the provider's connection limit.
